...
In addition to these changes,**many object properties have been added, which may help to produce more precise data, or data which would be easier to process**. The inverse properties wera also created, though we are not mentioning them below:
...
- rico:hasOrHadPart (with domain rico:Thing);
